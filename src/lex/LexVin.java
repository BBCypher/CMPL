...
public class LexVin extends Lex {
	...
	public LexVin(InputStream flot) {
		...
		super(flot);
		System.out.println("Binome JAFFRE-Paul_ADJI-Chene"); // TODO : maj avec vos noms
		System.out.println("--------------");
	...
	private int lireIdent() {
		  String res = "";
          do {
                  res = res + carLu; lireCarLu();
          }
          while((carLu >= 'a' && carLu <= 'z') ||(carLu >= 'A' && carLu <= 'Z'));
          
          int index = tabIdent.indexOf(res);
          if(index == -1) {
        	  tabIdent.add(res);
        	  int nvIndice = tabIdent.indexOf(res);
        	  numIdCourant = nvIndice;
        	  return IDENT ;
          }
          else {	
        	  if(index<NBRES) {
	 ...
	 */
	public int lireSymb() {
		// Les espaces ou assimiles sont "avales"
		while (carLu == ' ')
            lireCarLu();
            ...
            switch (carLu) {
                    ...
                    default : lireCarLu();return AUTRES;
            }
	...
	public String chaineIdent(int numIdent) {
		...
		return tabIdent.get(numIdent); // TODO
	...
	private void testeur_lexical() {
		...
		do {
			...
			if (token == NBENTIER)
				...
				Lecture.attenteSurLecture("token : " + images[token] + " attribut numIdCourant = " + numIdCourant + "chaine : " + this.chaineIdent(numIdCourant));
